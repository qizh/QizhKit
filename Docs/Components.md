...
The `Localizations` folder contains `.xcstrings` catalogs for app strings and unit names. Because the package declares `defaultLocalization = "en"` and processes this directory as a resource, you can bundle translations for any client of the library without a separate resource target.
