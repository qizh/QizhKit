...
///
/// When adopted, `ButtonStyleModifier` injects the environment `selected` flag so the style
/// can visually indicate selection.
...
///
/// When adopted, `ButtonStyleModifier` provides the environment `font` for typography-aware
/// sizing, spacing, or styling decisions.
...
///
/// When adopted, `ButtonStyleModifier` updates `isHovering` as the pointer enters or leaves
/// the view area, enabling hover-specific visuals.
...
/// A `ViewModifier` that applies a given `ButtonStyle` to content while enriching the style
/// with relevant environment-driven context when the style opts in.
///
...
///
///   - If the style conforms to `ButtonStyleDisableable`,
///     it receives `isEnabled` from the environment.
///   - If the style conforms to `ButtonStyleColorSchemeDependable`,
///     it receives the current `colorScheme`.
///   - If the style conforms to `ButtonStyleHighlighted`,
///     it receives the environment `highlightColor`.
///   - If the style conforms to `ButtonStyleSelectable`,
///     it receives the environment `selected` state.
///   - If the style conforms to `ButtonStyleCurrentFontAware`,
///     it receives the current environment `font`.
///   - If the style conforms to `ButtonStyleHoverable`,
///     it receives `isHovering` tracked by the modifier.
///   - If the style conforms to `ButtonStylePlaceholderable`,
///     it receives `isPlaceholder` derived from
///   `redactionReasons.contains(.placeholder)`.
///   - If the style conforms to `ButtonStyleKnowingPixelLength`,
///     it receives the environment `pixelLength`.
///
/// - Behavior:
///
...
/// - Usage:
///
///   - Apply via the convenience API on `ViewModifier`:
///     `content.modifier(.buttonStyle(MyStyle()))`, or directly initialize
...
public struct ButtonStyleModifier<Style: ButtonStyle>: ViewModifier {
	...
	/// Creates a `ButtonStyleModifier` that applies the given `ButtonStyle` to the content,
	/// enriching the style with relevant environment-driven values when the style type
	/// opts into them via the optional capability protocols.
	///
	...
	///   - ``ButtonStyleHoverable``: receives `isHovering` (tracked by the modifier)
	///   - ``ButtonStylePlaceholderable``: receives `isPlaceholder` (from `redaction` `reasons`)
	...
	/// - ``ButtonStylePlaceholderable``: injects `isPlaceholder` (from `redaction` `reasons`)
...
extension ViewModifier {
	...
	/// that opt into them.
	///
	/// This convenience constructor is intended to be used via the `ViewModifier` extension
	/// as: `SomeView.modifier(.buttonStyle(MyStyle()))`, or when used as a type-constrained
	/// modifier in places where a concrete `ViewModifier` type is required.
	///
	/// - Enriching `ButtonStyle`:
	///   
	...
	/// - Returns: A `ButtonStyleModifier` configured with the provided style, enriched with
	///   environment-driven state when supported by the style type.
	///
	/// - Usage:
	///   - Prefer this when you want your custom ButtonStyle to react to environment-driven
	///     changes like color scheme, enabled state, hover, or redaction without manually
	///     wiring them up.
